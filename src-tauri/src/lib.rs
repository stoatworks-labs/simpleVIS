...
///
/// This lives in the library rather than in `main.rs` because on iOS and
/// Android there is no `main` to run — the platform shell links this crate
/// in and calls the symbol the `mobile_entry_point` macro exports. The
/// desktop binary calls it directly, so both shells run the same setup.
